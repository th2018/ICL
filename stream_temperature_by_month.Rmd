...
```{r}
#siteNumber <- "13092747" 
#USGS 13269000 SNAKE RIVER AT WEISER ID
siteNumber <- "13269000" 
...
allQ <- readNWISdv(siteNumber,parameterCd,"2007-10-01","2015-09-30")
...
#allT <- readNWISdv(siteNumber,parameterCd,"2007-10-01","2015-09-30")

allT$yr<- format(allT$Date, format = "%Y") 
allT$mo<-as.numeric(format(allT$Date, format = "%m"))
summerT<-allT[allT$mo %in% c(6,7,8,9),]
head(summerT)
...
```{r}
plot(summerT$mo,summerT$X_00010_00003)

summerT$mo<-as.character(summerT$mo)

ggplot(data=summerT,aes(x=mo, y=X_00010_00003   ))+ geom_boxplot()+ggtitle(ChoptankInfo$station_nm) + ylab("Stream temperature (degrees Celsius)") + xlab("Month")
