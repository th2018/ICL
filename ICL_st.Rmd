...
```{r}
# read data
usgs<-read.dbf("usgs_wt_ID_nearest.dbf")
#usgs<-read.dbf("C:/Users/taohuang/Documents/Tao/Data/norwest/usgs_wt_ID_nearest.dbf")

#CREATE AN EMPTY DATA FRAME
meanAugT_df<-data.frame(agency_cd= " ",site_no="", Date=as.Date("2000-01-01",format="%Y-%m-%d"),X_00010_00003=0,    X_00010_00003_cd= "",yr="",mo=0 )
...
meanAugT<-meanT[meanT$mo==8,c("agency_cd","site_no","Date","X_00010_00003","X_00010_00003_cd","yr","mo"  )]
...
#save(meanAugT_df,file="meanAugT_df.Rdata")
...
for (id in  meanAugT_df$site_no) {
  if(length(meanAugT_df[meanAugT_df$site_no==id,]$X_00010_00003) >= (2021-2012+1)*31*0.9 ){
    p<-  qplot(meanAugT_df[meanAugT_df$site_no==id,]$X_00010_00003,
               geom="histogram",
      binwidth = 1,  
      main =  paste("USGS",id, ", skewness=", round(  skewness( meanAugT_df[meanAugT_df$site_no==id,]$X_00010_00003) , digits = 2), ", Coefficient of Variation=",  round(  cv( meanAugT_df[meanAugT_df$site_no==id,]$X_00010_00003))  )       , 
      xlab = "Stream T (deg C)",  
      fill=I("blue")
#,      col=I("red"), 
#      alpha=I(.2),
...
old_t$model<-"NorWeST predicted stream T for 1993-2011"
colnames(old_t)<-c("site_no","t","model" )

new_t<-meanAugT_df_usgs[,c("site_no","mean_T" )]
new_t$model<-"USGS observed stream T for 2012-2021"
colnames(new_t)<-c("site_no","t","model" )

models<-rbind(old_t,new_t)


ggplot(models, aes( model, t))+ geom_boxplot(notch = TRUE)+ 
  ylab("Stream T (deg C)")+
  xlab(" ") +
...
ggplot() +
  ...
  xlab("NorWeST predicted stream T for 1993-2011 (deg C)") + 
  ylab("USGS observed stream T for 2012-2021 (deg C)") +
  ggtitle(  paste(    "Mean August Temperature, correlation coefficient=", round( cor$estimate , digits = 2)   ))+
    theme(text = element_text(size=14))
